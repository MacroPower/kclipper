...
func (e *export) KCLSchemaToJSONSchema(pkgPath, schemaName string) ([]byte, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w: available schemas: %v", kclerrors.ErrSchemaNotFound, schemaKeys)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("%w: %w", kclerrors.ErrYAMLMarshal, err)
	}

	refYAML, ok := refYAMLAny.(map[string]any)
	if !ok {
		return nil, fmt.Errorf("%w: unexpected type %T for schema YAML", kclerrors.ErrInvalidFormat, refYAMLAny)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("%w: %w", kclerrors.ErrJSONMarshal, err)
	}
