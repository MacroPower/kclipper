...
//
// Vendored from github.com/charmbracelet/x/exp/teatest/v2 with the bubbletea
// import updated to charm.land/bubbletea/v2.
//
// MIT License - Copyright (c) 2023 Charmbracelet, Inc.
package teatest
...
// WithInitialTermSize sets the initial terminal size for the test model.
...
func WaitFor(
	...
	err := doWaitFor(r, condition, options...)
	if err != nil {
		tb.Fatal(err)
	}
}
...
func doWaitFor(r io.Reader, condition func(bts []byte) bool, options ...WaitForOption) error {
	...
	for time.Since(start) <= wf.Duration {
		_, err := io.ReadAll(io.TeeReader(r, &b))
		if err != nil {
...
func NewTestModel(tb testing.TB, m tea.Model, options ...TestOption) *TestModel {
	...
	programOpts := make([]tea.ProgramOption, 0, len(opts.programOpts)+4)
	programOpts = append(programOpts, opts.programOpts...)
...
func (s *safeReadWriter) Read(p []byte) (int, error) {
	...
	n, err := s.rw.Read(p)
	if err != nil {
		return n, err //nolint:wrapcheck // Preserve io.EOF for callers.
	}
